...
class Runner(object):
    ...
    def load_data(self):
        ...
        def get_data_loader(dataset_class, split, batch_size, shuffle=True):
            # input_dataset = dataset_class(self.triples[split], self.p)
            # data_sampler = torch.utils.data.distributed.DistributedSampler(dataset_class(self.triples[split], self.p))
            # print(dataset_class(self.triples[split], self.p))
            thedata = dataset_class(self.triples[split], self.p)
            data_sampler = torch.utils.data.distributed.DistributedSampler(thedata)
            return  DataLoader(
    ...
    def __init__(self, params):
        ...
        self.scheduler = MultiStepLR(self.optimizer, milestones=[0,1000], gamma=0.2)
    ...
    def add_model(self, model, score_func):
        ...
        self.init_entity_embed = None
        model = GGPNForLinkPrediction(self.edge_index, self.edge_type, self.init_entity_embed, None, params=self.p)
    ...
    def predict(self, split='valid', mode='tail_batch'):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(train_iter):
                ...
                #     a = torch.unique(pred[i])
                #     print(a.shape, pred[i].shape)
                #     assert(a.shape != pred[i].shape)
                target_pred		= pred[b_range, obj]
                pred 			= torch.where(label.byte(), -torch.ones_like(pred) * 10000000, pred)# filter
    ...
    def fit(self):
        ...
        for epoch in range(self.p.max_epochs):
            ...
            # if val_results['mr'] < self.best_val_mr:
            # if val_results['hits@10'] > self.best_hits:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-gcn_drop',	dest='dropout', 	default=0.5,  	type=float,	help='Dropout to use in GCN Layer')
