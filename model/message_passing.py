    ...
    def propagate(self, aggr, edge_index, **kwargs):
        ...
        for arg in self.message_args:
            if arg[-2:] == '_i':					# If arguments ends with _i then include indic
                ...
                # print("********************", tmp[edge_index[0]].size(), tmp[edge_index[0]])
            elif arg[-2:] == '_j':
        ...
        # print(message_args)
        # k, out, out_rel = self.message(*message_args)
        k, out = self.message(*message_args)
        
        k_add = scatter_(aggr, k, edge_index[0], dim_size=size)
        k_base = k_add[edge_index[0]]
        assert not torch.isnan(k).any()
        assert not torch.isnan(k_base).any()
        k = torch.div(k+k_base, k_base)
        assert not torch.isnan(k).any()
        ...
        # if out_rel != None:
        #     out_rel = k * out_rel
        #     out_rel = scatter_(aggr, out_rel, edge_type, dim_size=kwargs['rel_embed'].size(0))
